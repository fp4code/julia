...
@testset "union!, symdiff!" begin
    ...
    @test 0 ∈ symdiff!(IntSet(rand(1:100, 30)), 0)
    @test IntSet(0:2:4) ⊆ symdiff!(IntSet(rand(5:100, 30)), [0, 2, 4])
...
@testset "push!, union" begin
    ...
    s2 = IntSet()
    @test s2 === union!(s2, s1)
    s3 = IntSet([-1, 1, 10, 100])
    union!(s3, [-10, 1, 606, 1000])
    s4 = union(IntSet([-1, 1, 100, 1000]), IntSet([-10, 10, 100, 606]))
    @test s1 == s2 == s3 == s4
end
...
@testset "extreme values" begin
    @test pop!(IntSet(typemin(Int))) == typemin(Int)
    @test pop!(IntSet(typemax(Int))) == typemax(Int)
end
